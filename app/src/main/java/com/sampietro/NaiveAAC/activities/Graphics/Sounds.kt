 ...
 * <h1>Sounds</h1>
 *
 * **Sounds**  contains filepaths for sounds.
 */
open class Sounds : RealmObject() {
    /**
     * word or phrase.
     */
    @Index
...
//
            val FILE_NAME = "sounds.csv"
            // Grab all data from the DB in question :
            val resultsDB = realm.where(Sounds::class.java).findAll()

            // Here we need to put in header fields
            var dataP: String?
            val header = AdvancedSettingsDataImportExportHelper.grabHeader(realm, "Sounds")
        ...
        fun exporttoCsv(
            context: Context?,
            realm: Realm,
            soundsDescriptionSet: HashSet<String?>,
            dir: File?
        ) {
//
            val FILE_NAME = "sounds.csv"
            // Grab all data from the DB in question :
            val soundsDescriptionArray = arrayOfNulls<String>(soundsDescriptionSet.size)
            soundsDescriptionSet.toArray(soundsDescriptionArray)
            val realmQuery = realm.where(Sounds::class.java)
        ...
        fun importFromCsvFromInternalStorage(context: Context, realm: Realm, mode: String?) {
            if (mode == "Replace") {
                // clear the table
                val daCancellare = realm.where(Sounds::class.java).findAll()
                    ...
                    if (isTheFirstLine) {
                        ...
                        if (oneWord[1] != null) {
                            ...
                            if (oneWord[0] != null && oneWord[0]!!.length > 0 && oneWord[2] != null && oneWord[3] != null && f.exists()) {
                                realm.beginTransaction()
                                val sounds = realm.createObject(Sounds::class.java)
                                // set the fields here
                                sounds.descrizione = oneWord[0]
                                sounds.uri = uri
                                sounds.copyright = oneWord[2]
                                sounds.fromAssets = oneWord[3]
                                realm.commitTransaction()
                            }
                        }
                    }
                ...
                while (true) {
                    ...
                    if (isTheFirstLine) {
                        ...
                        if (oneWord[1] != null) {
                            // clear the table
                            val daCancellare =
                                realm.where(Sounds::class.java).equalTo("descrizione", oneWord[0])
                                    .findAll()
